...
func (r *MigrationHierarchyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if migrationObject.Object.(*danav1.MigrationHierarchy).Status.Phase != danav1.Complete {
		...
		// the root sns for resync every sns childrens displayname
		snsToSync, err := utils.NewObjectContext(ctx, log, r.Client, client.ObjectKey{Name: ns.GetName(), Namespace: utils.GetNamespaceParent(ns.Object)}, &danav1.Subnamespace{})
		if err != nil {
			return ctrl.Result{}, err
		}
		// get all the sns descendants and resync each one
		snsDescendants := utils.GetAllSnsChilds(snsToSync)
