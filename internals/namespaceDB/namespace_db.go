...
func MigrateNsHierarchy(ndb *NamespaceDB, client goclient.Client, log logr.Logger, snsname string, targetnsname string, ctx context.Context) error {
	...
	ns, err := utils.NewObjectContext(ctx, log, client, types.NamespacedName{Name: snsname}, &corev1.Namespace{})
	if err != nil {
		return err
	}
